...
int PC::equip(Dungeon &d, char c)
{
  ...
  if(index < CARRY_CAPACITY && !carry[index]) {
    ...
    switch(carry[index]->symbol){
	  ...
	  break;
	  case '~':
	  learn_spell(*carry[index]);
      carry[index] = NULL;	
	  d.message = "You learned a new spell!";
...
int PC::move(Dungeon &d, int c)
{
  switch(c) {
    ...
    break;
    case (int) '>':
    d.nummon = 0; // Causes regeneration of monsters
    return 1;
    case (int) '<':
    d.nummon = 0; // Causes regeneration of monsters
    return 1;
    ...
    if(!equipment[RANGED]){
      ...
      display_ranged_attack(d,
		equipment[RANGED]->damage_bonus.roll(),
		equipment[RANGED]->attribute);
    }
	break;
	case 's':
	display_spells(d);
...
int PC::learn_spell(Object &o)
{
	if(num_spells < MAX_SPELLS) {
		spell_names[num_spells] = o.name.c_str();
		spell_dice[num_spells] = &o.damage_bonus;
		spell_range[num_spells] = o.attribute;
		num_spells++;
	}	
