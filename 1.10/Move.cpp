...
int move_npc(Dungeon &d, NPC &c, uint8_t debug)
{
	if(!debug) {
		c.move(d);
	}
...
int ranged_combat(Dungeon &d, uint8_t tx, uint8_t ty, uint8_t r, uint8_t c, uint8_t damage)
{
	if(r != ty || c != tx) {
		damage = damage/(sqrt((r-ty)*(r-ty)+(c-tx)*(c-tx)*5));
	}
	...
	char buffer[81];
	sprintf(buffer, "You hit a monster dealing %d damage", damage);
	d.message = buffer;
