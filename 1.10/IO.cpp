...
int display_ranged_attack(Dungeon &d, uint32_t dam, uint32_t range)
{
  uint8_t cmd = ' ', tx = d.player.x, ty = d.player.y;
  ...
  do{
    mvprintw(0, 0, d.message.c_str());
    display_map(d);
    for(r = ty-range; r < ty+range+1; r++){
		for(c = tx-range; c < tx+range+1; c++) {
			if(!d.characters[r][c]){
				mvaddch(r+1, c, '^');	
			}
		}
    ...
    if(d.characters[ty][tx]) {
		attron(COLOR_PAIR(d.characters[ty][tx]->color));
		mvaddch(ty+1, tx, '*');
		attroff(COLOR_PAIR(d.characters[ty][tx]->color));
	}
  ...
  for(r = ty-range; r < ty+range+1; r++){
	for(c = tx-range; c < tx+range+1; c++) {
		if(d.characters[r][c]){
			if(ranged_combat(d, tx, ty, r, c, dam) == 10)
				return 10;			
		}
	}
  }
...
int display_spells(Dungeon &d)
{
	uint8_t i;
	mvprintw(0, 0, "Select a spell to cast, or ESC to exit");

	char buffer[50];
	mvprintw(2, 20, "	  Spells	  ");
	for(i = 3; i < 8; i++) {
		mvprintw(i, 20, "				");
	}
	for(i = 0; i < d.player.num_spells; i++) {
		sprintf(buffer, "[%d] %s-> DAM: %s RANGE: %d  ", i+1,
		d.player.spell_names[i], d.player.spell_dice[i]->c_str(),
		d.player.spell_range[i]);
		mvprintw(3+i, 20, buffer);
	}
	...
	switch(cmd - '0'){
	case 1:
		if(d.player.num_spells > 0) {
			display_ranged_attack(d, d.player.spell_dice[0]->roll(),
				d.player.spell_range[0]);
		}		
		break;
	}
